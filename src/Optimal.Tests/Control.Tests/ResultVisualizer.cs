...
namespace Optimal.Control.Tests
{
    ...
    {
        /// <summary>
        /// Generates an HTML file with interactive charts showing the optimal control solution.
        /// </summary>
        /// <param name="result">The collocation result to visualize.</param>
        /// <param name="problemName">Name of the problem (used in title and filename).</param>
        /// <param name="stateLabels">Labels for each state variable (e.g., ["h", "v", "m"]).</param>
        /// <param name="controlLabels">Labels for each control variable (e.g., ["T"]).</param>
        /// <param name="outputPath">Optional output path. If null, saves to bin/test/results/visualizations/.</param>
            ...
            string? outputPath = null)
        {
            ...
            var defaultDir = Path.Combine(
                ...
                "bin", "test", "results", "visualizations");
            ...
            var html = BuildHtmlContent(result, problemName, stateLabels, controlLabels);
            ...
            string[] controlLabels)
        {
            ...
            sb.AppendLine("        .container { max-width: 1400px; margin: 0 auto; background: white; padding: 30px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }");
            ...
            sb.AppendLine("            <canvas id=\"statesChart\"></canvas>");
            sb.AppendLine("        </div>");

            // Control trajectories chart
            sb.AppendLine("        <h2>Control Trajectories</h2>");
            sb.AppendLine("        <div class=\"chart-container\">");
            sb.AppendLine("            <canvas id=\"controlsChart\"></canvas>");
            sb.AppendLine("        </div>");

            // JavaScript for charts
            sb.AppendLine("        <script>");
            sb.AppendLine(GenerateChartScript(result, stateLabels, controlLabels));
            sb.AppendLine("        </script>");
            ...
            string[] controlLabels)
        {
            ...
            // Prepare state datasets
            var stateColors = new[] { "#FF6384", "#36A2EB", "#FFCE56", "#4BC0C0", "#9966FF", "#FF9F40" };
            var stateDatasets = new StringBuilder();
            var stateDim = result.States.Length > 0 ? result.States[0].Length : 0;

            for (var i = 0; i < stateDim; i++)
            {
                var label = i < stateLabels.Length ? stateLabels[i] : $"x{i}";
                var color = stateColors[i % stateColors.Length];
                var data = string.Join(", ", result.States.Select(state => state[i].ToString("F6")));

                if (i > 0)
                {
                    stateDatasets.Append(",\n");
                }
                stateDatasets.Append($@"                {{
                    label: '{label}',
                    data: [{data}],
            ...
            // Prepare control datasets
            var controlColors = new[] { "#E74C3C", "#3498DB", "#2ECC71", "#F39C12", "#9B59B6" };
            var controlDatasets = new StringBuilder();
            var controlDim = result.Controls.Length > 0 ? result.Controls[0].Length : 0;

            for (var i = 0; i < controlDim; i++)
            {
                var label = i < controlLabels.Length ? controlLabels[i] : $"u{i}";
                var color = controlColors[i % controlColors.Length];
                var data = string.Join(", ", result.Controls.Select(control => control[i].ToString("F6")));

                if (i > 0)
                {
                    controlDatasets.Append(",\n");
                }
                controlDatasets.Append($@"                {{
                    label: '{label}',
                    data: [{data}],
            ...
            sb.AppendLine("        // States Chart");
            sb.AppendLine("        const statesCtx = document.getElementById('statesChart').getContext('2d');");
            sb.AppendLine("        new Chart(statesCtx, {");
            sb.AppendLine("            type: 'line',");
            sb.AppendLine("            data: {");
            sb.AppendLine($"                labels: [{times}],");
            sb.AppendLine("                datasets: [");
            sb.AppendLine(stateDatasets.ToString());
            sb.AppendLine("                ]");
            sb.AppendLine("            },");
            ...
            sb.AppendLine("                    title: { display: false }");
            sb.AppendLine("                },");
            ...
            sb.AppendLine();
            sb.AppendLine("        // Controls Chart");
            sb.AppendLine("        const controlsCtx = document.getElementById('controlsChart').getContext('2d');");
            sb.AppendLine("        new Chart(controlsCtx, {");
            sb.AppendLine("            type: 'line',");
            sb.AppendLine("            data: {");
            sb.AppendLine($"                labels: [{times}],");
            sb.AppendLine("                datasets: [");
            sb.AppendLine(controlDatasets.ToString());
            sb.AppendLine("                ]");
            sb.AppendLine("            },");
            sb.AppendLine("            options: {");
            sb.AppendLine("                responsive: true,");
