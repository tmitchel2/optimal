...
internal static class RadiantBrachistochroneVisualizer
{
    ...
    private const double Xf = 10.0; // Final horizontal distance (must match solver)
    ...
    /// x = s = index / (n-1) * Xf (horizontal distance)
    /// y = n (vertical drop, positive = down)
    /// </summary>
    private static (float screenX, float screenY) GetScreenPosition(double[][] states, int index)
    {
        var numPoints = states.Length;
        var s = (double)index / (numPoints - 1) * Xf; // Horizontal distance
        var n = states[index][IdxN]; // Vertical drop (positive = descended)

        // Convert to screen coordinates
        // x maps to horizontal, n maps to vertical (positive n = lower on screen in Radiant's y-up system)
        var screenX = (float)(s * ScaleX) - 450.0f;
        var screenY = (float)(-n * ScaleY) + 100.0f; // Negate n because Radiant y-axis points up
    ...
    private static void DrawBead(Radiant.Graphics2D.Renderer2D renderer, double[][] states, int frameIndex)
    {
        ...
        var vx = (float)(v * Math.Cos(alpha)) * velocityScale;
        var vy = (float)(-v * Math.Sin(alpha)) * velocityScale; // Negate for screen coords
        renderer.DrawLine(
            new Vector2(x, y),
    ...
    private static void DrawInformation(Radiant.Graphics2D.Renderer2D renderer, double[][] states, double[][] controls, int iteration, double cost, double maxViolation, double constraintTolerance, int frameIndex)
    {
        ...
        var s = (double)frameIndex / (states.Length - 1) * Xf;

        renderer.DrawText($"FRAME: {frameIndex + 1}/{states.Length}", 280, TopY, 2, Colors.Amber400);
        renderer.DrawText($"POSITION: s={s:F2}m, n={state[IdxN]:F2}m", 280, TopY + 20, 2, Colors.Purple400);
